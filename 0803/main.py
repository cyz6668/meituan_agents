...
UPLOAD_DIR = "uploads"
MAX_CONCURRENT_TASKS = 8  # 根据GPU/CPU资源调整
THREAD_POOL_SIZE = 16     # I/O密集型任务的线程池大小
IMAGE_PROCESSING_EXECUTOR = concurrent.futures.ThreadPoolExecutor(max_workers=THREAD_POOL_SIZE)
MODEL_EXECUTOR = concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_TASKS)
...
async def judge(
    ...
    # task_id = str(uuid.uuid4())
    # # from celery_worker import judge_task
    # # result = judge_task.apply_async(args=[comment_text, comment_img_path, reply_text, reply_img_path,related_text], task_id=task_id)
    model_call = partial(
            call_ernie_multimodal_judge,
        ...
        # 使用单独的线程池执行模型推理（避免阻塞I/O线程池）
    result = await run_in_threadpool(model_call) #, executor=MODEL_EXECUTOR)
